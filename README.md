...
- [Почему moulinette поставила -42 за функцию из libft](#error_fillit)
- Настройка vscode
- Настройка clion
...
Администрация раздает тижи в зависимости от проступков. В итоге вы можете отрабатывать 8 часовой тиж, каждый день приходя в 11:00 выполняя задание по 2 часа в течение 4 дней.
...
>Перевод правил нормы на русский вы можете найти - [ТУТ](https://github.com/burninggoose/42-norme-ru)
...
Статические переменные ипользуются без ограничений, глобальные с учётом обоснования (см. разд. II.1 Правил). Обратите внимание, что в некоторых проектах использование глобальных переменных прямо запрещено.  
Необснованное использование глобальной переменной: счётчики, переменная использованная в одной функции. Обоснованное использование: ссылка на структуру хранения, например символьный буфер.
...
В теле функции комментарии не допускаются. Коментарии в стиле С99 - //, запрещены. (см. разд. II.11 Правил).
Правлильно оформленный комемнтарий выглядит так:  
  /*  
  ** Текст комментария.  
  */
...
<a name="error_fillit">Почему moulinette поставила -42 за функцию из libft</a>
---------------
Это происходит, потому что moulinett неправильно обрабатывает хедеры, которые построены подобным образом:
